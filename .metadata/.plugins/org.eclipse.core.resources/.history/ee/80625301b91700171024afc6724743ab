...
public String getCompletness(String parameter1, String parameter2) throws SQLException{
 	...
 	Gson gson = new GsonBuilder().create();
 	ArrayList responseArray = new ArrayList();
 	ResultSet rs = stmt.executeQuery( "SELECT DISTINCT f.validation_rule_id,dim2.validation_rule_comment,dim2.quality_metric_type_name,f.fact_row_no,dim1.service_main_group_name,f.fact_col_no,dim2.expected_result_amt,f.measure_amt,dim1.service_domain_name, dim2.expected_result_cnt,f.run_dttm FROM mesa.PL_MEASURE_FACT_PRT f INNER JOIN mesa.PL_SERVICE_PRT dim1 ON (f.validation_service_shortname = dim1.Service_Component_ShortName) INNER JOIN mesa.PL_VALIDATION_RULE_EXT dim2 ON (f.Validation_Rule_Id = dim2.Validation_Rule_Id) WHERE f.country_shortname='EE' AND dim1.service_main_group_name='BB Data Warehouse' and f.fact_row_no=1 AND dim2.quality_metric_type_name='COMPLETENESS' ;" );
 	while(rs.next()) {
 	  //int numColumns = rs.getColumnCount();
